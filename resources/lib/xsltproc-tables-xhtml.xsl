...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                version="1.0">
  <!--
      This file is identical to "xsltproc-tables-html.xsl" except for
      the default namespace.
  -->

  <xsl:param name="no.table.summary">0</xsl:param>

  <xsl:include href="./dumpfragment.xsl"/>
  <xsl:include href="./html-dtbl.xsl"/>
            ...
            <xsl:when test="$use.extensions != 0 and $tablecolumns.extension != 0">
              <xsl:choose>
                ...
                <xsl:when test="function-available('dtbl:convertLength')">
                  <xsl:copy-of select="dtbl:convertLength($table.width)"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:message terminate="yes">
